...
<header class="bg-custom text-white">
    ...
    <div class="container mt-5">
        ...
        <?php if (!empty($products)): ?>
                ...
                <tbody>
                    <?php foreach ($products as $item_id => $product): ?>
                        <tr id="row-<?php echo $item_id; ?>">
                            ...
                            <td>
                                <input type="number" value="<?php echo $product['quantity']; ?>" min="1" onchange="updateCart(<?php echo $item_id; ?>, this.value, <?php echo $product['price']; ?>)">
                                <button class="btn btn-danger btn-sm" onclick="removeFromCart(<?php echo $item_id; ?>)">-</button> <!-- Remove button -->
                            </td>
                            <td id="total-<?php echo $item_id; ?>"><?php echo format_price($product['total']); ?></td>
                            <td>
                                <form action="/remove_from_cart/<?php echo $item_id; ?>" method="POST">
            ...
            <h3>Total Price: <span id="grand-total"><?php echo format_price($total_price); ?></span></h3>
        <?php else: ?>
